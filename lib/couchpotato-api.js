...
CouchPotatoAPI.prototype._request = function _request(actions) {
	...
	if ((actions.parameters) && (actions.method == 'GET')) {
		var apiUrl = this.serverApi + actions.relativeUrl + jsonToQueryString(actions.parameters);
	}
	...
	if (actions.method == 'GET') {
		_.extend(headers, { "Accept": "application/json" });
	} else {
	...
	if (this.auth) {
		_.extend(headers, { "Authorization": 'Basic ' + new Buffer(this.username + ':' + this.password).toString('base64') });
	}
	...
	if (this.ssl) {
		_.extend(options, { "strictSSL": false });
	}
	
	// append the method parameter to the request
	_.extend(options, { "method": actions.method });

	if (_.contains(["POST", "PUT", "DELETE"], actions.method)) {
		_.extend(options, { "json": actions.parameters });
	} else {
		_.extend(options, { "json": true });
	}
